...
def insert(temp_close, temp_air, brightness):
    ...
    try:
        ...
        connection.commit()
        logger.info(f"[O] Succes de la requete, {cursor.rowcount} ligne(s) inseree")
    # Handle error
    except Exception as error :
        logger.error(f"[X] Echec de la requete : {error}")
    finally:
...
def select_all():
    ...
    try:
        ...
        connection.commit()
        logger.info(f"[O] Succes de la requete, {cursor.rowcount} ligne(s) recuperee")
        records = cursor.fetchall()
    # Handle error
    except Exception as error :
        logger.error(f"[X] Connection error : {error}")
    finally:
        # Closing database connection.
        if(connection.is_connected()):
            connection.close()
...
def delete_old():
    ...
    try:
        ...
        cursor.execute(request)
        connection.commit()
        logger.info(f"[O] Succes de la requete, {cursor.rowcount} ligne(s) effacee")
    # Handle error
    except Exception as error :
        logger.error(f"[X] Echec de la requete : {error}")
    finally:
...
def truncate():
    ...
    try:
        ...
        cursor.execute(request)
        connection.commit()
        logger.info(f"[O] Succes de la requete, table vide")
    # Handle error
    except Exception as error :
        logger.error(f"[X] Echec de la requete : {error}")
    finally:
